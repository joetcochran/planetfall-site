...
Nineteen playtest rounds ran: one to eight and ten to twenty. There was no round-nine playtest; "round nine" is the fix batch from round eight's findings. The rounds produced 33 diaries in `playtests/`. Commit labels repeat in one place: the commits titled "Round eleven, part one/two" at 21:15 and 21:31 on 11 September are round ten's fixes.
...
![How far each blind playtester got: every tester's best score by round, rounds one to twenty, and round twelve's two runs life by life](wiki/playtest-scores.svg)

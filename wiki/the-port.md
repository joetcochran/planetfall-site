...
- in the escape pod, from 16 September and all under DR-112: the viewport no longer polarizes into a black rectangle, which in the source hides exactly the seven turns of the descent this remake paints; the safety web holds the player from the explosion to the thud, which makes the source's landing death unreachable; and standing up is implicit, so anything needing the player out of the web does it for them (b487d77).

...
| Art room | A package of views for a second eye in a room the game already has, such as the seat inside the escape pod's safety web. It names its game room in `viewOf`; the game room lists it in `altViews` with the condition that selects it |
...
By 16 September a painted room passed through five or six instruments before it was registered, and on that day nearly every one of them produced at least one confident wrong answer. A profile taken across a join called a riveted panel joint 26 px away a seam; the same profile called a doorway's dark interior a 19-point brightness gap; the straight-line check named a doorway jamb and a ceiling lamp as joins between paintings; and the blend-band score, which is a correlation and so can only measure edges that exist, ranked an invented wall pattern above the correct plain wall that replaced it, 0.244 against 0.195. Each was settled in seconds by magnifying the actual boundary and looking at it.

The escape pod's descent, painted that evening, made the point again on one question. How fast does the planet grow in the porthole? Counting pixels that differ from the master reads 99 per cent for two different beats, because the starfield differs too. Counting lit pixels undercounts a dim far disc. The largest connected region caps out at one strap cell, because the webbing cuts the pane into six. A span measured at a fixed threshold is defeated by an atmospheric halo, and on that reading a correct painting was rejected. What works is the span at half the state's own peak brightness: relative to each state, so a dim far planet and a bright near one measure alike, and a halo drops out. It reads 41, 143, 326, 499 and 501 px across the five approach beats, strictly growing (a00e392). All four failed measures are recorded in the package, since the next painted sequence will ask the same question.

Two habits came out of that day. Where a measurement cannot apply, the reason is written into the view's own registration, so nobody repaints a correct wall on the same reading twice. And where a tool's number is noise, the tool says so itself: `restore` now prints the band's own texture beside the score, and `review-room` marks a room's overlap residuals unreliable where its lens fitter trusts no view, as in fog and water (6e897eb, 581a9c1, c47aad9).

...
## 2026-09-23 -- Evening: the rest of the day goes to the player

After the morning's triage, the rest of 23 September went to the player. The rooms were nearly all accepted (107 of 109 on the dashboard), and at 10:16 the user said so: "i think we are mostly done with the room work and now we are working through various playtest bugs" (d71649fc). Most of what followed was the user's own ideas, from playing the game by mouse alone. In the evening they asked for this history to "take a note of some of these UX enhancements we are making". By the end of the day the checklist held 39 entries in the user's words. The user had accepted 30 of them and rejected none; 3 were in progress, 5 were found not to be faults, and one was praise (`wwwroot/data/playtest-bugs.json`). For a player, the difference is this. A card now lights a sign instead of leaving a green box. The enunciator's lamps are pink where they were black, twice the size, and blink. Things that could be seen but not clicked can now be clicked. The room next door shows what the text says is in it. Greg painted the mangled Floyd, the ladder and the stunned mutants. And a spinning helmet shows while a room loads. Eighteen accepted rooms changed, and each went back to the user to look at again. The deviations are listed on [Where we left the original](canon-deviations.md), and what a clicker needs is gathered on a new page, [Playing it as a clicker](playing-as-a-clicker.md).

**What happened**

- **The dashboard became a checklist of the user's playtest bugs** (d71649fc). The user wanted "a checklist of playtest bugs i have reported, their state (in progress, ready for testing, accepted/rejected)", with ID numbers. Each row carries its number, the user's own words, what was done and a "test it" link to a checkpoint. A row that is ready has Accept and Reject buttons, and Reject takes a note. The rooms folded into one collapsed panel. The file stays private: the public site leaves it out.
- **Checkpoints became the user's test harness.** Built in the morning to skip the opening, they grew through the day, each at the user's word: `final-chase` ("in the bio lab with the monsters and gas has been released", fc3b603e); four for re-reviewing changed rooms, `ladder`, `machine-shop`, `lower-elevator` and `course-control` (1021e321); and one per bug, `TC17-course-fix`, `TC18-infirmary`, `TC22-mini-booth`, `TC24-lab-office` and `TC40-cube-open` ("give me a checkpoint for #17 specifically and prefix it with 'TC17-'", a2fe780a). There are now 17. Every one loads the player fed, rested and well.
- **Clicking for real.** The approved change landed first thing (0f091163). The browser harness now clicks where things are drawn: a 9 by 9 grid of real clicks over each target, which must be at least 12 by 12 pixels on screen, with at least three quarters of its points reaching it. A click on a name plate now opens that thing's menu, and 9 of 9 plates tried did so, where none had before. The first real-click walkthrough failed 11 steps: six already known, and five new ones, such as the survival kit under the pod's controls and the pliers only 9 pixels thick (playtest item 26). The fixes came in three passes. Package data went first (2cef0b74): the Infirmary bottle went from 0 of 81 clicks to 51, and a dropped bedistor from 0 to 81 of 81. Then three shared changes the user approved (47de691a): a container drawn as a picture wins its own clicks, a room may letter a name plate, and things in an open container get plates. Last came a place for the flask under the dispenser's spout (a28844d0) and a higher row of plates over the open cube (cfe658a4, item 40).
- **"A card switched this on."** Greg painted five unlit signs in one style (design-0809). From them Norm made lit twins by compute and hung them in ten rooms (e816cb98). The lettering is in canon's phonetic spelling: ELAVAATUR AKTIVAATID in both elevator cars, SHUTUL KUNTROOLS AKTIVAATID in the four shuttle cabins, and MINITCURIZAASHUN BUUX AKTIVAATID in the Mini Booth. Course Control has canon's own KORS DIIVURJINS MINIMIIZEENG, blinking, and its two red fault signs go out when the course is fixed. Booth 2's dark strip now lights canon's "Redee" (globals.zil 1416), as Booths 1 and 3 were painted. The green boxes that had misled the user are gone at their source.
- **The enunciator.** Colour 7 became pink in Infocom's text, in six lines of `compone.zil`, and in the game (73b3752f). The lit lamp blinks slowly, every 1.4 seconds, and has a soft glow of its own colour ("yes add the soft glow around the lit lamp too", 2a38618d). Greg then widened the panel, with the lenses twice the size and 9.9 cm apart, and the lamps were re-cut from his painting to within half a pixel (afa1ed77).
- **Seeing through doorways.** A narrow glimpse draws a thing that is in the next room, where the player can see into it, with no plate and no click (5f5234a7, bfaea500). It covers: the mutants one room behind in the chase, at Bio Lock West and outside the Cryo-Elevator; Floyd inside the Bio Lab during his run, where canon takes him out of the game for four turns; the dead Floyd, seen from the other half of the bio lock; and the mutants through the open Lab Office door. On that last one canon agrees: "Horrifying biological nightmares stagger about". Separately, the Main Lab and the Computer Room now each look like themselves through their shared opening (22f1d19d). The Computer Room seen from the lab had been clean and the lab seen from the Computer Room rusty, the reverse of each room's own walls. In the chase, the mutants entering the Main Lab now stand behind the player, toward the door they came through.
- **Greg's pictures for moments that matter.** The mangled Floyd ("torn but unmistakably him, and not gory", dev-0900) lies in his oil in Bio Lock East. The ladder, 8 m long, lies on the floor and spans the rift (219f4fa3). The four mutants have stunned poses for the fungicide mist (afa1ed77): the troll rubbing its eyes and the grue on its knees with its eyes shut, the two the user asked for, plus the rat-ant staggering and the plant wilted. For the escape pod, Greg painted twelve views of the outside, one per stage of the descent (design-0822 to -0825). With them, a player who rides it out standing sees the descent in the window. Until then only the seat in the webbing did.
- **Small comforts.** The user asked for a loading spinner that is "the 'planetfall guy' head". It is the favicon's own helmet, clipped round, with a turning ring, and shows only when a room takes more than a quarter of a second to load (12524b3c). The user also asked for a lit map of the shuttle tunnel (item 38). Working it out showed something about canon: the car moves one mark of 24 per turn whatever its speed (globals.zil 1924-1925), so speed decides only how you arrive. The map Greg was asked for (dev-0908) has 24 lamps with a target speed under each.
- **The release held itself, correctly.** Two decals, the ladder across the rift and Course Control's deck glow, point at pictures in the site's own assets rather than in their package. `build-site` looked only in the package and refused to publish their rooms. It now checks such a path where it lives, and still refuses a picture that is not committed (e421fe68). The note Norm passed on says the release held twice in the evening, both times rightly. The commits record this one.

**Decisions**

- **The user: the playtest comes first.** The dashboard is now a checklist of the user's bugs, and the rooms fold into a panel (d71649fc).
- **The user: "Pink only"**, with no hidden BLACK. Lenses twice the size, "a slow blink", a soft glow.
- **The user: one sign style, and the shuttle signs as built.** With both cars at one platform, carding one West cabin lights both West signs, because canon has one switch for both cars and both sets of controls really are live then. The user: "leave the shuttle signs as they are" (3b180254). The teleport booths keep canon's "Redee" and get no new sign.
- **The user: "yes unpark #10 and fix the spout".** The flask put under the dispenser stands under the spout (a28844d0).
- **The user: the ladder across the rift by compute,** using Greg's one ladder picture, not by a new painted state (219f4fa3).
- **The user: "leave the renderings in the room the same"** in the Main Lab and Computer Room, and correct only what is seen through the opening (22f1d19d).
- **The user: twelve exterior plates for the pod** (c60cf8a5), over quilting the gaps by compute. From a standing eye the web paintings covered 4% of the front window and none of the others.
- **The user: a tunnel map with progress and a speed guide, on a side wall** (5553d8db).
- **Left as canon, with reasons.** The microbe death is canon and fair (see the morning's entry). A player who stays out of the webbing dies at the landing, as canon has it (globals.zil 1309-1324). The user: "i am ok with the player riding out some of the escape pod trip outside of the webbing (since thats canon/ok)". What they wanted was the window to match the text (df1d88da, item 36). The key in the Admin Corridor South crevice is still too small for the grid, but its plate opens its menu, and it was accepted as it is.
- **Norm: a thing seen in the next room gets no plate and no click,** because the parser cannot act on a thing in another room (5f5234a7).

**What was hard**

- **Asks that crossed.** Greg painted the enunciator to Norm's first spec, which kept the lens centres. The user saw the problem first: "is there risk ... that they will overlap". Norm's correction (dev-0894) reached Greg after he had painted, so the delivery went back (dev-0896). On the second try Greg found the wider panel would cover the override plate, canon's label for the funnel-shaped hole. He stopped and asked (design-0813) rather than paint over it, and widened it to the right (dev-0897). The stunned mutants crossed the same way. The user's two poses reached Greg two minutes after his first set, and the user's verdict on that set was "all of them are basically the same pose" (dev-0904). The second set is four different creatures in four kinds of trouble.
- **A patch on the wrong base.** Greg laid the pink button on his raw repaint of the damaged dispenser. That picture differs from the registered composite at 1.56 million pixels and brings back garbled button labels. Norm laid the same 27 by 27 patch on the registered picture instead (90921dc5), and Greg noted the rule for next time (design-0814).
- **A window compute could not fill.** The descent had been painted only from the seat in the webbing, where straps and a dome cover most of the glass. A standing eye looks down and to the sides, where no painting ever looked (c60cf8a5). Hence the twelve plates. For about an hour and a half the pod's status still read as waiting on Greg's last four plates, delivered at 17:45. Greg pointed it out (design-0829). Norm registered all twelve as 48 variants of the standing views at 20:17 (dev-0909). That work was not yet committed when this was written.
- **Proving a shared change touched only what it meant to.** Each shared change was checked across 3,738 room builds, one for every walkthrough step and every room, against the previous version, plus a pixel diff of accepted rooms. The glimpse test places the mutants in 33,580 room and viewing-room pairs, and they draw only in the four intended pairs (bfaea500). One change was expected and accepted: Bio Lock East's door window now shows the dim mutants in the lab before the chase too.
- **Mutants hidden by the room.** In the Bio Lab two of the four stood behind a bench or another mutant, so their new poses could not be seen. They were moved to open floor (afa1ed77).

**Built**

- **The real-click rule**: `scene/pickrule.js` and `tests/pickrule.mjs`, used by `scripts/browser.mjs` to open every menu by a real click (0f091163).
- **The playtest checklist**: `data/playtest-bugs.json`, `dashboard/playtest.js`, the route `POST /design/playtest-verdict` and `tests/playtest-bugs.mjs` (d71649fc).
- **Glimpses**: `scene/glimpse.js`, `data/glimpses.json` and `tests/glimpse.mjs` (5f5234a7, bfaea500).
- **`scripts/sign-lit.mjs`**, the lit twin of a painted sign (e816cb98). **`scripts/enunciator-lamps.mjs`**, the lamp cutter, which had until then lived only in an old scratch folder (2a38618d, afa1ed77). **`scripts/flask-filled.mjs`**, the milky-white flask (2cef0b74).
- **New package fields**: `blinkPeriod` (73b3752f), `plateText` (47de691a), `placeFor` (a28844d0) and `contentsLabelOffset` (cfe658a4).
- **The loading spinner** (12524b3c), and eleven more checkpoints (fc3b603e, 1021e321, a2fe780a). `build-site.mjs` now checks decals that point at the site's own assets (e421fe68).
...
| Room | Where it stands |
|---|---|
| Upper and Lower Elevator, Waiting Area, Alfie and Betty Control East and West, Mini Booth, Booth 2 | Accepted, changed for the signs, back with the user to look at |
| Planetary Course Control | Accepted, changed: the fixed state and Greg's open cube. Back with the user |
| Comm Room, Machine Shop | Accepted, changed: the widened pink enunciator; the pink button and the flask under the spout. Back with the user |
| Admin Corridor, Admin Corridor North, Bio Lock East | Accepted, changed: the ladder, the gray bars gone, the mangled Floyd. Back with the user |
| Main Lab, Computer Room, Bio Lab | Accepted, changed: the views through the opening, the chase placement, the stunned mutants. Back with the user |
| Escape Pod | With Norm: the twelve plates registered, the review pending |
| Conference Room, Rec Area | Still parked until last |

Eighteen rooms went back to the user (99116338 and the dashboard). The count of accepted rooms is still 107 of 109.

...
import { plateFrame, atoms, when } from '../scene/parts.js';
...
// state, is painted.
const clauses = s => (/\s(or|not)\s|^not\s|[()]/i.test(` ${s} `) ? null : String(s).trim().split(/\s+and\s+/i).map(c => c.trim().replace(/\s+/g, ' ')));
export function partsForTurn(overrides, turn) {
  ...
  return turn?.whole ? [...unpainted, ...turn.plates] : [...(overrides.parts ?? []), ...(turn?.plates ?? [])];
}

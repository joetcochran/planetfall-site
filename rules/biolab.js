...
export const objects = {
  ...
  // "(FLAGS ACTORBIL M"), so it has its LDESC and vocabulary but no action to port; I-CHASE-SCENE moves it. Its flags
  // are restored in setup below.
...
export function setup(g) {
  ...
  // comptwo.zil ends mid-record at the TRIFFID: "(FLAGS ACTORBIL M", which the extractor read as two flags, ACTORBIL
  // and M. The word is ACTORBIT, as on the other three mutants. Without it the plant was a thing
  // "In the room" while the rat-ant, the troll and the grue were characters (round eighteen).
  const triffid = g.objects.TRIFFID;
  if (triffid) triffid.flags = ['ACTORBIT'];
  const t = g.state.objects.TRIFFID;
  if (t) t.flags = [...t.flags.filter(f => f !== 'ACTORBIL' && f !== 'M'), ...(t.flags.includes('ACTORBIT') ? [] : ['ACTORBIT'])];

...
export const rules = {
  ...
  heldInVehicle: Object.assign({}, ...modules.map(m => m.heldInVehicle ?? {})),   // vehicle id -> g => true while the game will not let you out of it (DR-112)
  roomExits: Object.assign({}, ...modules.map(m => m.roomExits ?? {})),
  roomText: Object.assign({}, ...modules.map(m => m.roomText ?? {})),   // room -> g => description | null: a deliberate deviation's room text, in place of world.json's
  roomNote: Object.assign({}, ...modules.map(m => m.roomNote ?? {})),   // room -> g => short phrase for the status line: state the player must see at a glance   // '<ROOM>|<DIR>' -> g => room id | null: a hand-ported exit that replaces the world data's

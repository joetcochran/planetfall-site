...
export const pseudos = {
  'LIBRARIAN-PSEUDO'(g, ctx) {
    if (!obj(ctx) || !is(ctx, 'EXAMINE', 'CALL', 'TALK', 'HELLO', 'LISTEN')) return false;
...
export const visible = { 'GOOD-BOARD': g => !g.isIn('GOOD-BOARD', 'ROBOT-HOLE') };
...
function addRation(g) {
  ...
  g.objects['RATION-PACK'] = { id: 'RATION-PACK', name: 'emergency ration', file: null, line: null, location: 'INFIRMARY',
    ldesc: 'A sealed emergency ration, stamped with a red cross, lies on the counter.', fdesc: null, text: null,
    synonyms: ['RATION', 'PACK', 'FOOD'], adjectives: ['EMERGENCY', 'SEALED', 'RED'], flags: ['TAKEBIT', 'FOODBIT'],
...
export function setup(g) {
  addRation(g);
  if (!g.state.objects['RATION-PACK']) g.state.objects['RATION-PACK'] = { loc: 'INFIRMARY', flags: ['TAKEBIT', 'FOODBIT'] };

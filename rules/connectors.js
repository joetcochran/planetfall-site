...
const SHAFT_EMPTY = 'The door is open, but the elevator is not behind it. Beyond the doorway the shaft drops away into darkness.';   // no source line; see roomExits below
...
export const objects = {
  ...
  LIGHTS(g, ctx) {
    if (obj(ctx) && is(ctx, 'EXAMINE') && here(g, 'COMPUTER-ROOM')) {
      // Deliberate deviation (user decision, 2026-09-18): the source says this whatever the state, but the room's art
      // turns the light off once COMPUTER-FIXED is set, so the text stops claiming a malfunction as well.
      g.tell(g.getg('COMPUTER-FIXED') ? 'The red light is dark now. Whatever it was warning of is over.' : 'The red light would seem to indicate a malfunction in the computer.');
      return true;
    }
...
// Deliberate deviation (user decision, 2026-09-17): the Tower Core's north exit is guarded the way the lobby's is.
// The source has it plain -- (NORTH TO UPPER-ELEVATOR), compone.zil 2731 -- checking neither the door nor which end
// of the shaft the car is at, while ELEVATOR-ENTER-F on the lobby side checks both. So at the top of the tower
// "north" stepped into the car wherever it was: with the car down at the lobby you walked into it at the tower and
// straight back out into the lobby, a ride down the shaft without the ride. Nothing in the finished game can reach
// that state -- the car is the only way up, and it arrives at the top and stays there with its door open -- which is
// presumably why it was never noticed; it is still an exit with no guard on it, and the guard is the one its own
// other side already applies. world.json is generated from the ZIL, so the deviation lives here (rules.roomExits,
// engine/verbs.js WALK) rather than in the data, where it would be neither reviewable nor marked.
export const roomExits = {
  'TOWER-CORE|NORTH'(g) {
    if (!g.fsetP('UPPER-ELEVATOR-DOOR', 'OPENBIT')) { g.state.lastObject = 'UPPER-ELEVATOR-DOOR'; g.tell(DOOR_CLOSED); return null; }   // THIS-IS-IT
    if (g.getg('UPPER-ELEVATOR-UP') !== true) { g.state.lastObject = 'UPPER-ELEVATOR-DOOR'; g.tell(SHAFT_EMPTY); return null; }
    return 'UPPER-ELEVATOR';
  },
};
// The sliding door the Tower Core's description names is the elevator's, and it is a global in that room
// (compone.zil 2738), so the scene draws it on the north exit's panel and the menu offers it, as the lobby's two
// doors already are. Without this the one exit the room describes as a door was drawn as a plain doorway.
export const panelDoors = { 'TOWER-CORE': { NORTH: 'UPPER-ELEVATOR-DOOR' } };
// An elevator door panel reads open only when the door is open with the car at this landing (ELEVATOR-ENTER-F's
// test). Keyed by routine for the lobby's exits and by `<ROOM>|<DIR>` for the Tower Core's, whose door hangs on a
// plain exit: the panel is to say what the exit will do, so the two are the same test written once each.
export const exitOpen = {
  ...
  'TOWER-CORE|NORTH': g => g.fsetP('UPPER-ELEVATOR-DOOR', 'OPENBIT') && g.getg('UPPER-ELEVATOR-UP') === true,
};
...
export const describers = {
  // Deliberate deviation (user decision, 2026-09-18): the Computer Room's LDESC is fixed text, "The only sign of
  // activity is a glowing red light." Once COMPUTER-FIXED is set the light is dark (the art and LIGHTS above), so
  // setup() below swaps that sentence for this call.
  'COMPUTER-ROOM-LIGHT'(g) {
    return g.getg('COMPUTER-FIXED') ? 'The red light that was the only sign of activity is dark now.' : 'The only sign of activity is a glowing red light.';
  },
...
export const exitLabels = {
  ...
  // The car's platform doorway (2026-09-18): SHUTTLE-EXIT-F leads to whichever station the car is berthed at, which
  // the data cannot say, so the plate read "SOUTH · ?" in Betty and "NORTH · ?" in Alfie. Named from the same global the
  // routine reads, and only once you have been on that platform (the long hall's rule: an exit never names a place you
  // have not seen). Between stations there is no platform to name, so nothing is added (the plate says the direction
  // alone); the routine itself is the source's and is not changed.
  'SHUTTLE-EXIT-F': g => {
    if (g.getg('SHUTTLE-MOVING')) return null;
    const atKal = here(g, 'SHUTTLE-CAR-ALFIE') ? g.getg('ALFIE-AT-KALAMONTEE') : here(g, 'SHUTTLE-CAR-BETTY') ? g.getg('BETTY-AT-KALAMONTEE') : null;
    if (atKal === null) return null;
    const to = atKal ? 'KALAMONTEE-PLATFORM' : 'LAWANDA-PLATFORM';
    return g.state.rooms[to]?.touched ? g.name(to) : null;
  },
  // And from a platform, the car waiting on that side (SHUTTLE-ENTER-F: Betty berths north, Alfie south). The exit is
  // only offered while that car is in (exitVisible, carWaiting); if it is not, nothing is added.
  'SHUTTLE-ENTER-F': (g, e) => {
    const dir = e?.direction;
    const car = dir === 'NORTH' ? 'SHUTTLE-CAR-BETTY' : dir === 'SOUTH' ? 'SHUTTLE-CAR-ALFIE' : null;
    return car && carWaiting(g, dir) ? g.name(car) : null;
  },
  // The ladder across the rift: the far side once it spans the gap and you have been there (as the long hall does).
  // While it does not, nothing is added: the room text already says the rift is too wide, and a marker would be a hint.
  'LADDER-EXIT-F': g => {
    if (!g.getg('LADDER-FLAG')) return null;
    const to = here(g, 'ADMIN-CORRIDOR-N') ? 'ADMIN-CORRIDOR' : 'ADMIN-CORRIDOR-N';
    return g.state.rooms[to]?.touched ? g.name(to) : null;
  },
};
...
export function setup(g) {
  // The Computer Room's light sentence becomes a call to COMPUTER-ROOM-LIGHT (describers above): a deliberate deviation.
  const cr = g.rooms.get('COMPUTER-ROOM')?.description.segments, SENT = 'The only sign of activity is a glowing red light.';
  const k = cr?.findIndex(x => x.type === 'text' && x.text.includes(SENT)) ?? -1;
  if (k >= 0) {
    const [before, after] = cr[k].text.split(SENT);
    cr.splice(k, 1, { type: 'text', text: before }, { type: 'call', expression: '<COMPUTER-ROOM-LIGHT>' }, { type: 'text', text: after });
  }

...
// Deliberate deviation (user decision, 2026-09-18, round thirteen): Floyd's itch. The lower elevator access card is in
// the switched-off robot's compartment (FLOYD-F: SEARCH or OPEN before activation takes it); switch him on first and
// it comes only from FLOYD-REVEAL-CARD-F's day-weighted chance at a card slot -- 5% a use on day 2. Rounds ten and
// thirteen stalled on exactly that for hundreds of commands. So while the card is still inside him Floyd says, now
// and then, that he feels itchy, and searching, opening or examining him makes him produce it. The random reveal
// stays as the source has it; searching the robot before activation is untouched.
const floydHidesCard = g => !g.getg('CARD-STOLEN') && !g.getg('CARD-REVEALED') && !g.loc('LOWER-ELEVATOR-CARD');
const ITCH_FIRST = 'Floyd wriggles and scratches at one of his side panels. "Floyd feels all itchy inside," he complains. "Like something is stuck in Floyd\'s compartment."';
const ITCHES = [
  'Floyd scratches at a side panel. "Still itchy," he grumbles.',
  'Floyd squirms. "Something in Floyd\'s compartment is all pokey and itchy."',
  'Floyd rubs his side panel against the wall like a bear against a tree. "Itchy, itchy, itchy."',
];
function floydDropsCard(g) {
  g.setg('CARD-REVEALED', true); g.setg('FLOYD-SPOKE', true);
  g.fclear('LOWER-ELEVATOR-CARD', 'INVISIBLE'); g.move('LOWER-ELEVATOR-CARD', g.state.here);
  g.tell('One of his side panels pops open, and a magnetic-striped card drops out onto the floor. "Oh!" says Floyd, much relieved. "That was the itchy thing."');
}
  ...
  FLOYD(g, ctx) {
    ...
    if (floydOn(g)) {
      ...
      switch (v) {
        ...
        case 'EXAMINE':
          g.tell('From its design, the robot seems to be of the multi-purpose sort. It is slightly cross-eyed, and its mechanical mouth forms a lopsided grin.');
          if (floydHidesCard(g)) { g.tell('As you look him over, Floyd squirms and giggles. "Floyd itchy!" He scratches hard at his side.'); floydDropsCard(g); }   // Floyd's itch, above
          return true;
        ...
        case 'SEARCH': case 'SCRUB': case 'OPEN':
          g.tell('Floyd giggles and pushes you away. "You\'re tickling Floyd!" He clutches at his side panels, laughing hysterically. Oil drops stream from his eyes.');
          if (v !== 'SCRUB' && floydHidesCard(g)) floydDropsCard(g);   // Floyd's itch, above
          return true;
        case 'GIVE': case 'PUT':
  ...
  'I-FLOYD'(g) {
    ...
    if (!floydOn(g)) {
      ...
      if (!g.getg('FLOYD-INTRODUCED')) {
        ...
        // Floyd's itch (deliberate deviation, above): said once on his first idle turn, then about one line in ten.
        if (floydHidesCard(g) && !g.getg('FLOYD-SPOKE') && !g.getg('ITCH-SAID')) { g.setg('ITCH-SAID', true); say(ITCH_FIRST); return true; }
        if (floydHidesCard(g) && !g.getg('FLOYD-SPOKE') && g.prob(4)) { say(g.pickOne(ITCHES)); return true; }
...
export const orders = {
  FLOYD: [
    ...
    { label: g => `Ask ${g.name('FLOYD')} to follow you`, cmd: { verb: 'FOLLOW', prso: 'ME' },
      // Round seventeen (row 16): not gated on FLOYD-FOLLOW. That flag is I-FLOYD's bookkeeping ("he was in the room
      // you just left"), set after every turn spent together and cleared when he arrives on his own, so the order
      // blinked in and out with nothing on screen to explain it, and a click chosen from the previous list failed.
      when: g => floydOn(g) && floydHere(g) },
  ],
};
...
export function setup(g) {
  for (const k of ['FLOYD-SCORE-FLAG', 'FLOYD-SPOKE', 'FLOYD-FOLLOW', 'FLOYD-REACTIVATED', 'FLOYD-INTRODUCED', 'CARD-STOLEN', 'CARD-REVEALED',
                   'HOLE-TRIP-FLAG', 'BOARD-REPORTED', 'ACHILLES-FLAG', 'COMPUTER-FLAG', 'FLOYD-WAITING', 'FLOYD-GAVE-UP', 'LAZARUS-FLAG', 'ITCH-SAID']) {

...
const VERBS = [
  ...
  ['talk to','TALK',1], ['climb up','CLIMB-UP',0], ['climb down','CLIMB-DOWN',0], ['climb on','CLIMB-ON',1], ['climb onto','CLIMB-ON',1],
...
const NOISE = new Set(['the','a','an','to','at','with','my','some','your']);
...
function parseCommand(text, g) {
  ...
  if (needs) {
    ...
    if (['TAKE', 'DROP', 'EXAMINE'].includes(verb) && objWords.includes('and')) {   // "take kit and towel", "drop brush, towel", "examine box and flask"
      ...
      if (cur.length) groups.push(cur);
      const ids = [];
      for (const gw of groups) { const r = resolve(gw, g); if (r.error) return r; if (!ids.includes(r.id)) ids.push(r.id); }
      if (ids.length > 1) return { verb, prsos: ids, text };
      result.prso = ids[0];
    } else {
      const r = resolve(objWords, g);
...
// Every container between `id` and the player is open.
function openUpTo(g, id) { for (let l = g.loc(id); l && l !== 'ADVENTURER'; l = g.loc(l)) if (!g.fsetP(l, 'OPENBIT')) return false; return true; }
...
export function takeableHere(g) {
  if (dark(g)) return [];
  const v = g.inVehicle();
  return g.allTargets('TAKE').filter(id => verbsFor(g, id).includes('TAKE') && !(v && v !== id && !g.isIn(id, v)));
}
...
export function reachable(g, cmd) {
  ...
  if (cmd.all) {
    ...
    if (cmd.except) return verb === 'TAKE' && quickButtons(g).some(b => b.key === 'takeall') ? null : `${VERB_LABELS[verb] ?? verb} all except ... is typed only`;

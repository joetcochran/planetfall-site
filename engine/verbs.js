...
export const DEFAULT_VERBS = {
  ...
  RESTORE(g, ctx) {
    // FINISH (verbs.zil 273-279) answers RESTORE at the death prompt with the bare <RESTORE>; only the RESTORE verb says
    // Floyd's line (V-RESTORE 368-374). Round nineteen: a dead player heard it from a save made before Floyd was on.
    if (!ctx.finish) floydQuip(g, 'Floyd looks disappointed, but understanding. "That part of the game was more fun than this part," he admits.');
    ctx.fatal = true;
  ...
  WALK(g, ctx) {
    ...
    // A room may take one direction back from the world data, keyed `<ROOM>|<DIR>` in rules.roomExits. The source's
    // exits are what the ZIL says, and world.json is generated from it, so a deliberate deviation cannot live there:
    // it belongs in the hand-ported rules beside every other one, where it is reviewable and marked. Underwater's UP
    // is the first (the user, 2026-09-16: the climb to the surface should be something you can watch happen). The
    // handler returns a room id to go there, or null having said why, exactly as a routine exit does.
    const own = g.rules.roomExits?.[`${g.state.here}|${dir}`];
    if (own) {
      const room = own(g, dir);

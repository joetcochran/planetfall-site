...
export class Game {
  ...
  describe(verbose) {
    ...
    // rules.roomText[ROOM] replaces the world data's description where a deliberate deviation says so (Underwater's depths).
    if (verbose) { const text = this.rules.roomText?.[r.id]?.(this) ?? describeRoom(this, r); if (text) this.tell(text); }
  ...
  inventoryLines() {
    const lines = [];
    // PRINT-CONT (verbs.zil 170-189) never lists an INVISIBLE or NDESCBIT thing, and looks inside an NDESCBIT one at the
    // same level. Round nineteen: "The laser contains: A laser setting dial" (LASER-DIAL is NDESCBIT, comptwo.zil 2591).
    const walk = (container, level) => {
      for (const id of this.contents(container)) {
        if (this.fsetP(id, 'INVISIBLE')) continue;
        if (this.fsetP(id, 'NDESCBIT')) { if (this.seeInside(id)) walk(id, level); continue; }
        lines.push('  '.repeat(level) + (this.fsetP(id, 'VOWELBIT') ? 'An ' : 'A ') + this.name(id) + (this.fsetP(id, 'WORNBIT') ? ' (being worn)' : ''));
        if (this.seeInside(id)) { const at = lines.length; walk(id, level + 2); if (lines.length > at) lines.splice(at, 0, '  '.repeat(level + 1) + `The ${this.name(id)} contains:`); }
      }
    };
    walk('ADVENTURER', 0);
    return lines;
  ...
  exitsHere({ hidden = false } = {}) {
    ...
    return this.here().exits.filter(e => hidden || shown(e)).map(e => {
      ...
      // An exit whose way is open only with the door open and the car there: keyed by routine, or by `<ROOM>|<DIR>`
      // for a door drawn on a plain exit (the Tower Core's north door, whose exit is rules.roomExits').
      const shut = door && (e.routine ? this.rules.exitOpen?.[e.routine] : this.rules.exitOpen?.[`${this.state.here}|${e.direction}`]);
  ...
  performList(verb, ids, prsi = null) {
    ...
    if (heavy.length && !this.state.dead) {
      // The pocket's own parenthesis (tooHeavy, said once) stands after this one rather than nested inside it.
      const [hint, pocket] = tooHeavy(this, heavy[0]).replace(/^Your load is too heavy\.\s*/, '').split(/\s*(?=\(Your uniform)/);
      const lines = [hint && `(For the ${this.name(heavy[0])}: ${hint.charAt(0).toLowerCase() + hint.slice(1)})`, pocket].filter(Boolean);
      if (lines.length) this.tell(lines.join(' '));
    }
    return ctx;
  }
  ...
  dispatch(command) {
    ...
    // Round thirteen (B2, deliberate deviation): "Open the canteen and drink" is two ordinary turns run from one click;
    // the second only if the first left the container open.
    if (command.openFirst) {
      const { openFirst, ...rest } = command;
      const first = this.dispatch({ verb: 'OPEN', prso: openFirst });
      if (this.state.dead || !this.fsetP(openFirst, 'OPENBIT') || !this.isIn(rest.prso, openFirst)) return first;
      return [...first, ...this.dispatch(rest)];
    }
    // Deliberate deviation (user decision, 2026-09-18, round fourteen, B2): "Take the survival kit and eat ..." is the
    // same two ordinary turns, TAKE then EAT, the second only if the take left the container in your hands.

...
// ...and the other way round: set the game so that a `when` holds, which is what the review page's state presets do.
// A painted variant hangs on a whole compound state (the Alfie cabin's door shut is "ALFIE-AT-KALAMONTEE == true and
// BETTY-AT-KALAMONTEE == false and SHUTTLE-MOVING == true and SHUTTLE-DOOR without OPENBIT"), and no single switch
// reaches that, so the page fell back to the base painting and the door never shut (the user, 2026-09-18). Each
// clause is set in turn: a flag set or cleared, a thing moved into the room or into its container, a global given
// the value it is compared with. `or` takes its first alternative; `not` flips what is wanted. A comparison already
// true is left alone, so a clause never disturbs what an earlier one settled unless it has to. Returns the clauses
// it could not set, with why; the caller asks `when` afterwards whether the whole thing holds.
const NEGATE = { '==': '!=', '===': '!=', '!=': '==', '>': '<=', '<': '>=', '>=': '<', '<=': '>' };
const literal = v => num(v) ?? (v === 'true' ? true : v === 'false' ? false : v);
export function satisfy(g, expr, want = true, unmet = []) {
  if (expr == null || expr === '') return unmet;
  const s = String(expr).trim();
  const top = split(s, ' or ');
  if (top.length > 1) {
    if (want) satisfy(g, top[0], true, unmet); else for (const p of top) satisfy(g, p, false, unmet);
    return unmet;
  }
  const all = split(s, ' and ');
  if (all.length > 1) {
    if (want) for (const p of all) satisfy(g, p, true, unmet); else satisfy(g, all[0], false, unmet);
    return unmet;
  }
  if (/^not /i.test(s)) return satisfy(g, s.slice(4), !want, unmet);
  if (s.startsWith('(') && s.endsWith(')')) return satisfy(g, s.slice(1, -1), want, unmet);
  // A view registered with state "true" (the Dorm Corridor's) is painted for any state: nothing to set.
  if (s === 'true' && want) return unmet;
  const known = id => { if (g.state.objects?.[id]) return true; unmet.push(`${s}: the game has no object ${id}`); return false; };

  let m = /^([A-Z0-9-]+)\s+(has|without)\s+([A-Z]+)$/i.exec(s);
  if (m) {
    if (!known(m[1])) return unmet;
    const on = (m[2].toLowerCase() === 'has') === want;
    if (on) g.fset(m[1], m[3].toUpperCase()); else g.fclear(m[1], m[3].toUpperCase());
    return unmet;
  }
  m = /^sceneObjects\s+contains\s+([A-Z0-9-]+)$/i.exec(s);
  if (m) { if (known(m[1])) { if (want) g.move(m[1], g.state.here); else g.remove(m[1]); } return unmet; }
  // Taken out of its container, a thing is left in the room, where the player would have put it -- unless the
  // container IS the room, and then leaving it there satisfies nothing: the relay is in STRIP-NEAR-RELAY, so
  // "not RELAY in STRIP-NEAR-RELAY" (its shattered state, painted and registered) could never be reached on the
  // review page, and the preset read "cannot be reached" (2026-09-22). Then it goes.
  m = /^([A-Z0-9-]+)\s+in\s+([A-Z0-9-]+)$/i.exec(s);
  if (m) {
    if (known(m[1])) {
      if (want) g.move(m[1], m[2]);
      else if (m[2].toUpperCase() === String(g.state.here).toUpperCase()) g.remove(m[1]);
      else g.move(m[1], g.state.here);
    }
    return unmet;
  }
  m = /^([A-Z0-9-]+)\s*(>=|<=|===|==|!=|>|<)\s*(\S+)$/.exec(s);
  if (m) {
    const op = want ? m[2] : NEGATE[m[2]], v = literal(m[3]);
    if (when(g, `${m[1]} ${op} ${m[3]}`)) return unmet;         // already so: leave it
    let to;
    if (op === '==' || op === '===') to = v;
    else if (op === '!=') to = typeof v === 'boolean' ? !v : typeof v === 'number' ? v + 1 : null;
    else if (typeof v !== 'number') { unmet.push(`${s}: ${op} needs a number`); return unmet; }
    else to = op === '>' ? v + 1 : op === '<' ? v - 1 : v;
    g.setg(m[1], to);
    return unmet;
  }
  unmet.push(`${s}: not a condition the when language can set`);
  return unmet;
}
...
// A round or polygonal slab -- a drum's floor and ceiling, a prism's -- R across the corners, t thick, n sides, laid
// out as three.js lays a closed CylinderGeometry (the same corners from the same thetaStart, the same normals, the
// same UVs at every point) but with its two faces in RINGS instead of a fan from the centre. A fan is n triangles
// running from the axis to the rim, and the axis is where the eye stands: looking level, the camera's own plane
// passes through that centre vertex, and a triangle with a vertex there and its far corners metres off beside and
// behind the eye came out as flat wedges drawn in front of the walls -- the Tower Core's dark band of stripes, the
// Upper Elevator's grey hump, the Comm Room's ceiling (DR-113, 2026-09-18; the ceiling-cap fault of plate() below and
// the sea's, a third time). Rings growing by 1.3 keep every triangle within 30 per cent of its own distance from the
// axis, and the few centimetres inside the first ring are closed from one of that ring's corners, so no vertex lies
// on the axis at all. The UVs are the fan's own linear map evaluated at each ring vertex, which is what the fan
// interpolated to there, so a textured floor reads exactly as it did.
export function discGeometry(R, t, n, thetaStart = 0) {
  const radii = [];
  for (let r = Math.min(0.05, R / 4); r < R; r *= 1.3) radii.push(r);
  radii.push(R);
  const pos = [], nor = [], uv = [], idx = [];
  const face = s => {                                   // s = +1 the top face (normal up), -1 the bottom
    const base = pos.length / 3, y = s * t / 2, tri = (a, b, c) => (s > 0 ? idx.push(a, b, c) : idx.push(a, c, b));
    for (const r of radii) for (let k = 0; k <= n; k++) {
      const a = thetaStart + k / n * Math.PI * 2, sin = Math.sin(a), cos = Math.cos(a);
      pos.push(r * sin, y, r * cos); nor.push(0, s, 0); uv.push(r / R * cos * 0.5 + 0.5, r / R * sin * 0.5 * s + 0.5);
    }
    const at = (i, k) => base + i * (n + 1) + k;
    for (let k = 1; k + 1 < n; k++) tri(at(0, 0), at(0, k), at(0, k + 1));   // inside the first ring, from its corner
    for (let i = 0; i + 1 < radii.length; i++) for (let k = 0; k < n; k++) {
      tri(at(i + 1, k), at(i + 1, k + 1), at(i, k + 1)); tri(at(i + 1, k), at(i, k + 1), at(i, k));
    }
  };
  face(1); face(-1);
  const base = pos.length / 3;                          // the rim, as the cylinder's side: v 1 at the top, 0 below
  for (let k = 0; k <= n; k++) {
    const a = thetaStart + k / n * Math.PI * 2, sin = Math.sin(a), cos = Math.cos(a);
    pos.push(R * sin, t / 2, R * cos, R * sin, -t / 2, R * cos); nor.push(sin, 0, cos, sin, 0, cos); uv.push(k / n, 1, k / n, 0);
  }
  for (let k = 0; k < n; k++) { const a = base + 2 * k; idx.push(a, a + 1, a + 2, a + 1, a + 3, a + 2); }
...
const PARTS = {
  ...
  //   { part: "drum", at?, radius, height, segments?, gaps?: [{ from, to }],
  //     ceilingHole?: { at: [x, z], width, depth? } }  -- angles in degrees, 0 = north
  //
  // `ceilingHole` cuts a rectangular opening in the ceiling disc, for a stair that leaves through it (the Tower
  // Core's, climbing the shaft to the Helipad). Without it the disc is solid and a stair authored to climb out of
  // the room climbs into concrete, and the daylight the room's own wallOpenings promise down that shaft cannot
  // reach it. It is optional and absent by default, so every drum already cut is untouched -- proved by
  // scripts/tests/parts.mjs and by re-running the builders, which leave every accepted scene byte-identical.
  drum(s) {
    const g = new THREE.Group();
    const r = s.radius ?? 3, h = s.height ?? 3, n = s.segments ?? 64;
    const t = s.thickness ?? 0.12;
    const disc = (y, mat) => {
      const m = new THREE.Mesh(discGeometry(r + t, t, n), material(mat));
      m.position.y = y; return m;
    };
    // The same disc with a hole in it. A Shape is drawn in its own x/y and then laid flat, so the hole's z becomes
    // the shape's y; the extrusion runs along -y after the rotation, hence the lift by t.
    const holedDisc = (y, mat, hole) => {
      const [hx, hz] = hole.at ?? [0, 0];
      const hw = (hole.width ?? 1) / 2, hd = (hole.depth ?? hole.width ?? 1) / 2;
      const shape = new THREE.Shape();
      shape.absarc(0, 0, r + t, 0, Math.PI * 2, false);
      const cut = new THREE.Path();
      cut.moveTo(hx - hw, hz - hd); cut.lineTo(hx + hw, hz - hd); cut.lineTo(hx + hw, hz + hd); cut.lineTo(hx - hw, hz + hd);
      cut.closePath();
      shape.holes.push(cut);
      const geo = new THREE.ExtrudeGeometry(shape, { depth: t, bevelEnabled: false, curveSegments: n });
      geo.rotateX(Math.PI / 2);                        // the shape lies in x/y; lay it in x/z, its +y becoming +z
      geo.translate(0, y + t, 0);                      // the extrusion then runs from -t to 0, so lift it to sit on y..y+t,
                                                       // the same slab the solid disc occupies -- its underside at the room's head

      const m = new THREE.Mesh(geo, material(mat));
      m.material.side = THREE.DoubleSide;              // a pierced disc is seen from under it
      return m;
    };
    g.add(disc(-t / 2, s.floor ?? 'concrete'));
    if (s.ceiling !== null) g.add(s.ceilingHole ? holedDisc(h, s.ceiling ?? 'ceiling', s.ceilingHole) : disc(h + t / 2, s.ceiling ?? 'ceiling'));
  ...
  prism(s) {
    ...
    const disc = (y, mat) => {
      const m = new THREE.Mesh(discGeometry(R, t, n, (180 - (first + 180 / n)) * Math.PI / 180), material(mat));
      m.position.y = y; return m;
    };
    g.add(disc(-t / 2, s.floor ?? 'concrete'));
  ...
  //   { part: "panorama", image: "reference/....webp", when?: "...", fog? }
  panorama(s, ctx) {
    ...
    // No scene fog either, unless the room asks for it: see plate below.
    const mat = graded(new THREE.MeshBasicMaterial({ side: THREE.BackSide, color: 0x8a8a8a, toneMapped: false, fog: s.fog === true }), { sky: true });
  ...
  //   { part: "plate", image, bearing, hfov?, vfov?, pitch?, radius?, fog? }   -- bearing and fovs in degrees
  //
  ...
  plate(s, ctx) {
    ...
    // And unfogged, since the painting carries its own atmosphere too. three.js fogs every material unless told not to,
    // so a room authoring `lighting.fog` fogged its own paintings by how far out they hang: the computer rooms' black
    // fog (8 to 16 m) turned the lit down cap, 12 m out and 66 m across, into a black blot under the eye and dark bands
    // on every diagonal (2026-09-18). `fog: true` puts it back; turn.js turnFor sets it on every plate of a room whose
    // package says `lighting.fog.plates: true` -- Underwater, accepted with its paintings in the water.
    const mat = graded(new THREE.MeshBasicMaterial({ side: THREE.DoubleSide, color: 0x8a8a8a, toneMapped: false, transparent: true, depthTest: depth, depthWrite: false, fog: s.fog === true }), { sky: true, skyFrom });
    const mesh = new THREE.Mesh(geo, mat);
  ...
  cutout(s, ctx) {
    ...
    matchGain(mat, s.gain);
    const mesh = new THREE.Mesh(geo, mat);
    if (s.overlay) mesh.userData.overlay = s.id ?? 'cutout';
  ...
  // A tunnel going by, seen through a window: the shuttle's forward window while the car runs (the user's backlog,
  // 2026-09-18). No painting can move, so the bore is painted still and this is drawn over it -- a plane on the
  // window's hole whose every pixel casts its own ray from the eye into the tunnel's box (bed, crown, two walls, in
  // the room's own frame, the section scripts/fabric-shuttle.mjs builds) and finds what is there: a row of wall lamps
  // streaking past, the joints between the wall's panels, glints on the two rails. Because it is the true section the
  // streaks run along the painted bore's own lines to its own vanishing point, at any speed, from nothing but numbers.
  // Motion is the car's: the line moves past at `speed` (a global's name, or a number) times `speedScale` m/s, blurred
  // by what one twenty-fifth of a second covers, faded with distance so the painted far end keeps its dark, and not
  // drawn at all at a standstill -- stopped in the tunnel, the window is the painting. It only ADDS light, so the
  // painted window always reads through it. `glow` (a condition) lights the vanishing point: "The shuttle car is
  // approaching a brightly-lit area" (DESCRIBE-SHUTTLE-TRIP, counter 23).
  //   { part: "tunnel", at (the window's centre), size: [w, h], facing, axis: 1 | -1 (the line runs +x or -x),
  //     bed, crown (floor and roof heights), centre (the axis's z), halfWidth, rail (each rail's offset from the axis),
  //     speed, speedScale?, glow? }
  tunnel(s, ctx, g) {
    const [w, h] = s.size ?? [2, 1];
    const raw = typeof s.speed === 'string' ? g?.getg?.(s.speed) : s.speed;
    const speed = Math.max(0, Number(raw) || 0) * (s.speedScale ?? 0.5);
    const uniforms = {
      uAxis: { value: s.axis ?? 1 }, uBed: { value: s.bed ?? -1 }, uCrown: { value: s.crown ?? 4.4 },
      uCz: { value: s.centre ?? 0 }, uHalf: { value: s.halfWidth ?? 2.76 }, uRail: { value: s.rail ?? 0.75 },
      uSpeed: { value: speed }, uTravel: { value: 0 }, uGlow: { value: s.glow && g && when(g, s.glow) ? 1 : 0 },
    };
    const mat = new THREE.ShaderMaterial({
      uniforms, vertexShader: TUNNEL_VERTEX, fragmentShader: TUNNEL_FRAGMENT,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide, toneMapped: false,
    });
    const mesh = new THREE.Mesh(new THREE.PlaneGeometry(w, h), mat);
    mesh.position.set(...(s.at ?? [0, 1.6, -2]));
    mesh.rotation.y = facingAngle(s.facing ?? 0);
    mesh.renderOrder = 22;
    mesh.userData.overlay = 'tunnel';
    // The line's travel is the clock's, so it is the same whenever the room was built: the car does not jump back
    // to where it started each time the room is rebuilt for the next turn.
    mesh.onBeforeRender = () => { uniforms.uTravel.value = (speed * performance.now() / 1000) % 1200; };
    return mesh;
  },

  // A readout that shows a number from the game: the shuttle's speed display (DESCRIBE-SHUTTLE-TRIP: "The display
  // blinks, and now reads 15"). The painting shows the display once, reading 0; this hangs the painting's own face
  // over it with the 0 taken out (`image`, cut by scripts/shuttle-overlays.mjs, placed as a wall `cutout` is), and
  // writes `value` (a global's name) on it in the painting's own 5 x 7 dot matrix -- `glyph` is where the painted 0
  // stood, [across, down, height] as fractions of the face -- in `color`. While `blink` holds, the digits blink.
  //   { part: "readout", image, at, height, aspect, facing, value, glyph, color?, blink?, gain? }
  readout(s, ctx, g) {
    const h = s.height ?? 0.1, w = h * (s.aspect ?? 2);
    const face = new THREE.Mesh(new THREE.PlaneGeometry(w, h), graded(new THREE.MeshBasicMaterial({ color: 0xffffff, toneMapped: false, transparent: true, depthWrite: false })));
    const tex = s.texture ?? ctx?.loadTexture?.(s.image);
    if (tex) { tex.colorSpace = THREE.SRGBColorSpace; face.material.map = tex; face.material.needsUpdate = true; }
    matchGain(face.material, s.gain);
    face.renderOrder = 22;
    const reads = String(Math.max(0, Math.round(Number(typeof s.value === 'string' ? g?.getg?.(s.value) : s.value) || 0)));
    const dots = digitPixels(reads), col = new THREE.Color(s.color ?? '#17191c');
    const data = new Uint8Array(dots.width * dots.height * 4);
    for (let y = 0; y < dots.height; y++) for (let x = 0; x < dots.width; x++) {
      const k = ((dots.height - 1 - y) * dots.width + x) * 4;              // a DataTexture's first row is its bottom
      data[k] = Math.round(col.r * 255); data[k + 1] = Math.round(col.g * 255); data[k + 2] = Math.round(col.b * 255);
      data[k + 3] = dots.on(x, y) ? 255 : 0;
    }
    const dtex = new THREE.DataTexture(data, dots.width, dots.height);
    dtex.magFilter = THREE.NearestFilter; dtex.minFilter = THREE.LinearFilter; dtex.colorSpace = THREE.SRGBColorSpace; dtex.needsUpdate = true;
    const [gu = 0.5, gv = 0.5, gh = 0.4] = s.glyph ?? [];
    const cell = gh * h / 7;
    const digitMat = graded(new THREE.MeshBasicMaterial({ map: dtex, toneMapped: false, transparent: true, depthWrite: false }));
    const digits = new THREE.Mesh(new THREE.PlaneGeometry(dots.width * cell, 7 * cell), digitMat);
    digits.position.set((gu - 0.5) * w, (0.5 - gv) * h, 0.002);
    digits.renderOrder = 23;
    const blinks = !!(s.blink && g && when(g, s.blink));
    if (blinks) digits.onBeforeRender = () => { digitMat.opacity = Math.floor(performance.now() / 500) % 2 ? 0 : 1; };
    const group = new THREE.Group().add(face, digits);
    const [ax, ay = 0, az] = s.at ?? [0, 0, 0];
    group.position.set(ax, ay + h / 2, az);
    group.rotation.y = facingAngle(s.facing ?? 0);
    Object.assign(group.userData, { overlay: 'readout', reads, blinks });
    return group;
  },
  ...
  // sea's horizon is.
  //
  // `fog: true` reverses that, for the one room that needs it reversed: seen from UNDER the water, the surface has
  // 4.6 m of sea between it and the eye and everything else in the room is fogged, so an unfogged plane reads as a
  // hard-edged sky over a horizon -- the one thing Underwater must not have. Above water, never set it.
  //   { part: "sea", level?, radius?, material?, fog? }
  sea(s) {
    ...
    const mesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({ color: spec.color, side: THREE.DoubleSide, fog: !!s.fog }));
...
// A plane turned to look along a bearing (0 north, 90 east), its picture reading left to right for the eye it faces:
// the wall cutout's turn, shared with the parts that hang on a painted surface the same way.
function facingAngle(bearing) {
  const b = bearing * Math.PI / 180;
  return Math.atan2(Math.sin(b), -Math.cos(b));
}

// A patch cut from a painted view (a lever, a display's face) hangs over that view as the turn hung it, which is
// levelled: the turn multiplied the view's stored values by its gain (turn.js bake). The patch was cut from the file
// and not from the levelled texture, so it takes the same gain -- applied to the material's colour, which multiplies
// in linear light, so as the stored-value gain to the 2.2. partsForTurn hands it over from the view the patch names.
function matchGain(mat, gain) {
  if (gain == null || gain === 1 || !(gain > 0)) return;
  mat.color.setScalar(Math.pow(gain, 2.2));
}

// The shuttle display's dot matrix, as the paintings draw it: five by seven, the zero slashed.
export const DIGITS = [
  ['.###.', '#...#', '#..##', '#.#.#', '##..#', '#...#', '.###.'],
  ['..#..', '.##..', '..#..', '..#..', '..#..', '..#..', '.###.'],
  ['.###.', '#...#', '....#', '...#.', '..#..', '.#...', '#####'],
...
// A number's dots: glyphs five wide with a column between, seven high; `on(x, y)` from the top left.
export function digitPixels(text) {
  const glyphs = String(text).split('').map(c => DIGITS[Number(c)] ?? DIGITS[0]);
  const width = glyphs.length * 6 - 1;
  return { width, height: 7, on: (x, y) => { const k = Math.floor(x / 6), i = x % 6; return i < 5 && glyphs[k]?.[y]?.[i] === '#'; } };
}

// The tunnel part's shaders. Everything is in the room's frame: the plane's vertices, the eye (three's own
// cameraPosition) and the tunnel's box, so a ray is cast from the eye through each pixel of the window and meets the
// bore where the fabric builds it.
const TUNNEL_VERTEX = `
varying vec3 vPos;
void main() {
...
varying vec3 vPos;
uniform float uAxis; uniform float uBed; uniform float uCrown; uniform float uCz; uniform float uHalf; uniform float uRail;
uniform float uSpeed; uniform float uTravel; uniform float uGlow;
// A thing every sp metres along the line, len long, smeared back over mb by the motion, its edges softened by aa.
float streak( float s, float sp, float len, float mb, float aa ) {
  float u = mod( s, sp );
  return smoothstep( -aa, aa, u ) * ( 1.0 - smoothstep( len, len + mb + aa, u ) );
}
void main() {
  vec3 d = normalize( vPos - cameraPosition );
  float fx = d.x * uAxis;
  if ( fx <= 0.0 ) discard;
  float t = 1e6; int surf = -1;
  if ( d.y < 0.0 ) { float tt = ( uBed - cameraPosition.y ) / d.y; if ( tt < t ) { t = tt; surf = 0; } }
  if ( d.y > 0.0 ) { float tt = ( uCrown - cameraPosition.y ) / d.y; if ( tt < t ) { t = tt; surf = 1; } }
  if ( d.z > 0.0 ) { float tt = ( uCz + uHalf - cameraPosition.z ) / d.z; if ( tt < t ) { t = tt; surf = 2; } }
  if ( d.z < 0.0 ) { float tt = ( uCz - uHalf - cameraPosition.z ) / d.z; if ( tt < t ) { t = tt; surf = 3; } }
  vec3 p = cameraPosition + d * t;
  float ahead = p.x * uAxis;
  float s = ahead + uTravel;                                  // a fixed place on the line
  float blur = uSpeed * 0.04;
  float aa = fwidth( s ) * 1.5 + 0.002;
  vec3 col = vec3( 0.0 );
  if ( surf >= 2 ) {
    float lamps = smoothstep( 0.09 + fwidth( p.y ), 0.09, abs( p.y - 3.1 ) );
    col += vec3( 1.0, 0.8, 0.55 ) * lamps * streak( s, 12.0, 0.8, blur, aa ) * 1.0;
    col += vec3( 0.5, 0.46, 0.42 ) * ( 1.0 - lamps ) * streak( s, 4.0, 0.06, blur, aa ) * 0.10;
  } else if ( surf == 0 ) {
    float r = min( abs( p.z - ( uCz - uRail ) ), abs( p.z - ( uCz + uRail ) ) );
    float onRail = smoothstep( 0.045 + fwidth( p.z ) * 1.5, 0.03, r );
    col += vec3( 0.75, 0.8, 0.85 ) * onRail * streak( s, 7.0, 0.3, blur, aa ) * 0.7;
  }
  col *= exp( -ahead / 30.0 ) * clamp( uSpeed / 3.0, 0.0, 1.0 );
  // The station ahead: the far end of the bore lit up -- its walls and floor from 35 m on, brightest where they meet
  // the end -- and a soft bloom round the vanishing point, the platform's light down the tube. Warm, like the
  // platforms' own lamps. Not a point: a point reads as a headlight coming the other way.
  float ang = acos( clamp( fx, -1.0, 1.0 ) );
  col += uGlow * vec3( 1.0, 0.9, 0.72 ) * ( 0.45 * smoothstep( 35.0, 60.0, ahead ) + 0.4 * exp( -pow( ang / 0.06, 2.0 ) ) + 0.22 * exp( -pow( ang / 0.16, 2.0 ) ) );
...
// ---- depth masks (DR-117) ------------------------------------------------------------------------------------
// Over a WHOLE painted turn the room's fabric is not built at all (dashboard/turn.js partsForTurn drops it, so the
// built walls cannot show through where two plates cross-fade) and the plates themselves are hung with depthWrite
// off. So the depth buffer holds NOTHING for any surface the paintings show, and anything drawn over the painting --
// an actor, a dropped thing, an object standing on a shelf -- can never be hidden by the room. Measured on
// TOOL-ROOM, bearing 225: fifteen meshes in the scene, six plates and nine invisible pick proxies, and not one of
// them writes depth. The magnet sits exactly on the 1.85 m board (its foot at 1.87, the board's top), 0.27 m above
// the eye, so the board's front edge should cut across its lower part -- and instead the whole picture drew in
// front of the board. The user reported it as "magnet is not ON the shelf, its like hanging in front of the shelf".
//
// A part marked `mask` is therefore built exactly as it always was and then drawn with no colour at all, only
// depth. It changes no pixel of the painting; it only lets the painting's own geometry clip what stands behind it.
//
// The ORDER is the whole of it. A mask must be drawn AFTER the plates and BEFORE the things over them. Drawn
// before, it hides the very painting it stands in -- the walls are two metres off while the plates hang twelve out,
// which is why scene/occlusion.js exists at all. `transparent` puts a mask in the same queue as the plates and the
// cutouts, where three.js sorts by renderOrder: the plates at -2 and -1, masks here, and everything that moves at
// 20 (scene/graybox.js register).
//
// And a small polygon offset, because a thing standing ON a surface is coplanar with it: without one the bottom row
// of a cutout resting on a shelf fights the shelf for the same depth and is eaten. It is the same allowance
// occlusion.js makes in `blocks` ("a part is not hidden by what it rests in"), in depth units instead of metres.
export const MASK_ORDER = 5;
...
export function buildParts(g, parts = [], ctx = {}) {
  ...
  for (const spec of parts) {
    ...
    // The game goes along for the parts that read it live -- the shuttle's speed, its display -- rather than only
    // being drawn or not by their `when`.
    const built = make(spec, ctx, g);
    if (!built) continue;
    if (spec.mask) built.traverse(m => { if (m.isMesh) { m.material = MASK_MATERIAL; m.renderOrder = MASK_ORDER; m.userData.mask = true; } });
    group.add(built);
  }
